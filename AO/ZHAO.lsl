...
loadNoteCard() {
    ...
    if ( llGetInventoryKey(notecardName) == NULL_KEY ) {
        llOwnerSay( "Notecard '" + notecardName + "' does not exist." );
...
default {
   ...
   control( key _id, integer _level, integer _edge ) {
      if ( _edge ) {
         // For some reason, SL doesn't turn you around when you walk backwards, but does when you run.
         // This hack turns you around when you walk (but not run) backwards so your walking anim appears
         // more natural
         if ( _level & _edge & CONTROL_BACK ) {
